...
const STORAGE_KEY = "course1StudyReader.v3";
const LEGACY_STORAGE_KEY = "course1StudyReader.v2";
...
function applyReaderFont() {
  const size = clamp(Number(state.readerFont) || 19, 16, 23);
  ...
  const html = MANIFEST.modules.map((module) => {
    ...
    return `<section class="module-nav-group ${collapsed}" data-module-group="${module.id}">
      <button class="module-nav-head" type="button" data-toggle-module="${module.id}">
        <span><span>${escapeHtml(module.number)}</span><strong>${escapeHtml(module.title)}</strong></span><i aria-hidden="true">⌄</i>
      </button>
...
function renderReaderPage(page) {
  const parts = [];
  if (page.hasVisual) {
    const label = page.visualLabel || "La fuente original contiene contenido visual.";
    parts.push(`<aside class="reader-visual-cue"><span><strong>Referencia visual:</strong> ${escapeHtml(label)}. La versión exacta está disponible sin salir de la lectura.</span><button type="button" data-open-source="true">Ver página fuente</button></aside>`);
  }
...
function updateReaderChrome() {
  ...
  $("#readerSectionMeta").textContent = `Página fuente ${ui.currentPage} · ${position} de ${total} en esta sección · Reflow para lectura móvil`;
  ...
  $("#favoritePageButton").innerHTML = favorite ? "<span aria-hidden=\"true\">★</span> Guardada" : "<span aria-hidden=\"true\">☆</span> Guardar";
  $("#markPageButton").textContent = completed ? "Página completada ✓" : "Marcar página";
...
function renderPageNotes() {
  ...
  target.innerHTML = notes.slice(0, 3).map((note) => `<div class="note-mini"><strong>${escapeHtml(note.title)}</strong><span>${escapeHtml(note.body)}</span><button type="button" data-edit-note="${escapeHtml(note.id)}">Editar</button> <button type="button" data-delete-note="${escapeHtml(note.id)}">Eliminar</button></div>`).join("");
}
...
async function renderReader() {
  ...
  try {
    ...
    $("#pageFocusHint").textContent = page.hasVisual
      ? "Esta página incluye un cuadro, diagrama o diseño visual. Lee aquí y consulta la Fuente visual cuando quieras ver la forma original."
...
function renderSource() {
  ...
  $("#openNativePdfLink").href = `${MANIFEST.course.sourcePdf}#page=${ui.currentPage}`;
  updateReaderChrome();
}
...
function renderPracticeCard(exercise) {
  ...
  return `<article class="practice-card" data-exercise="${escapeHtml(exercise.id)}">
    ...
    <div class="practice-actions"><button class="primary-button" type="button" data-practice-action="verify">Verificar respuesta</button>${feedback?.checked ? `<button class="compact-button" type="button" data-practice-action="retry">Intentar otra vez</button>` : ""}</div>
    ${feedbackHtml}
    <footer class="practice-card-bottom"><span>${source} · ${escapeHtml(sectionForPage(exercise.page).title)}</span><button class="link-button" type="button" data-practice-action="source">Abrir referencia ↗</button></footer>
  </article>`;
}
...
function glossaryCard(entry) {
  ...
  return `<article class="glossary-card" data-term="${escapeHtml(entry.term)}">
    ...
    <div class="glossary-card-actions">
      <button class="term-action" type="button" data-glossary-action="translate" data-term="${escapeHtml(entry.term)}">Traducir</button>
      <button class="term-action" type="button" data-glossary-action="listen" data-term="${escapeHtml(entry.term)}">Escuchar</button>
      <button class="term-action ${status === "learned" ? "is-active" : ""}" type="button" data-glossary-action="learned" data-term="${escapeHtml(entry.term)}">${status === "learned" ? "✓ Aprendida" : "Aprendida"}</button>
      <button class="term-action ${status === "review" ? "is-active" : ""}" type="button" data-glossary-action="review" data-term="${escapeHtml(entry.term)}">${status === "review" ? "↻ En repaso" : "Repasar"}</button>
    </div>
  </article>`;
}
...
function renderGlossaryTerms(entries) {
  ...
  const more = entries.length > visible.length ? `<button id="glossaryLoadMore" class="compact-button glossary-load-more" type="button">Mostrar ${Math.min(24, entries.length - visible.length)} términos más</button>` : "";
  return grid + more;
}
...
function renderFlashcards(entries) {
  ...
  const origin = entry.origin || {};
  return `<div class="flashcard-wrap"><div class="flashcard-toolbar"><span>${ui.glossary.flashIndex + 1} de ${entries.length}</span><button class="link-button" type="button" data-flash-action="shuffle">Mezclar</button></div>
    ...
    <div class="flashcard-actions">${revealed ? "" : `<button class="primary-button" type="button" data-flash-action="reveal">Revelar definición</button>`}<button class="compact-button" type="button" data-flash-action="learned">La sé</button><button class="compact-button" type="button" data-flash-action="review">Necesito repasar</button><button class="compact-button" type="button" data-flash-action="next">Siguiente →</button></div>
  </div>`;
}
...
function renderSavedSelections() {
  ...
  return `<div class="saved-selection-grid">${words.map((item) => `<article class="saved-selection"><p>${escapeHtml(item.text)}</p><small>Guardado desde la página ${item.page}</small><div class="glossary-card-actions"><button class="term-action" type="button" data-personal-action="translate" data-id="${escapeHtml(item.id)}">Traducir</button><button class="term-action" type="button" data-personal-action="listen" data-id="${escapeHtml(item.id)}">Escuchar</button><button class="term-action" type="button" data-personal-action="source" data-id="${escapeHtml(item.id)}">Ir a lectura</button><button class="term-action" type="button" data-personal-action="delete" data-id="${escapeHtml(item.id)}">Eliminar</button></div></article>`).join("")}</div>`;
}
...
function bindEvents() {
  ...
  $("#fontDecreaseButton").addEventListener("click", () => { state.readerFont = clamp(state.readerFont - 1, 16, 23); applyReaderFont(); persistState(); });
  $("#fontIncreaseButton").addEventListener("click", () => { state.readerFont = clamp(state.readerFont + 1, 16, 23); applyReaderFont(); persistState(); });
  ...
  $("#visualReferenceButton").addEventListener("click", () => setView("source"));
